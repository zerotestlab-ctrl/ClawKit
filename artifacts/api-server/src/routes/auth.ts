...
import { hashPassword, comparePassword, createSessionToken, requireAuth, resolvePlanForEmail } from "../lib/auth.js";
...
router.get("/me", requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    res.json({
      ...
      email: user.email,
      name: user.name,
      plan: resolvePlanForEmail(user.email, user.plan),
...
router.post("/login", async (req: Request, res: Response) => {
  try {
    ...
    res.json({
      user: { id: user.id, email: user.email, name: user.name, plan: resolvePlanForEmail(user.email, user.plan), createdAt: user.createdAt },
